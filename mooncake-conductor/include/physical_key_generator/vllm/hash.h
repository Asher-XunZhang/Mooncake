...
inline std::vector<uint8_t> hex_to_bytes(const std::string& hex) {
    ...
    if (hex.length() % 2 != 0) {
        throw std::invalid_argument("The length of the hexadecimal string must be even.");
    }
    ...
    for (size_t i = 0; i < hex.length(); i += 2) {
        ...
        else throw std::invalid_argument("Invalid hexadecimal character: " + std::string(1, high));
        
        if (low >= '0' && low <= '9') byte |= (low - '0');
        else if (low >= 'a' && low <= 'f') byte |= (low - 'a' + 10);
        else if (low >= 'A' && low <= 'F') byte |= (low - 'A' + 10);
        else throw std::invalid_argument("Invalid hexadecimal character: " + std::string(1, low));
