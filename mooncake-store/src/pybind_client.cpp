...
tl::expected<Replica::Descriptor, ErrorCode> PyClient::getreplica(const std::string &key) {
    auto query_result = client_->Query(key);
    if (!query_result) {
        if (query_result.error() == ErrorCode::OBJECT_NOT_FOUND ||
            query_result.error() == ErrorCode::REPLICA_IS_NOT_READY) {
...
tl::expected<std::map<std::string, std::vector<Replica::Descriptor>>, ErrorCode> PyClient::batchgetreplica(const std::vector<std::string>& keys) {
    auto query_results = client_->BatchQuery(keys);
    if (query_results.size() != keys.size()) {
        LOG(ERROR) << "batchgetreplica response size mismatch.";
        return tl::unexpected(query_results[0].error());
    }
